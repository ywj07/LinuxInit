...
do
    ...
    echo "循环配置host:$num"
     ...
     /usr/bin/ansible $ip -m shell  -a " echo -e 'HOSTNAME=hadoop${num}.${DOMAIN_NAME}\n' >>  /etc/sysconfig/network" 
     /usr/bin/ansible $CLUSTER -m shell  -a " echo -e ' ${ip}  hadoop${num}.${DOMAIN_NAME}' >>  /etc/hosts"
...
/usr/bin/ansible $CLUSTER -m shell -a " echo -e ' */10 * * * *  /usr/sbin/ntpdate -u  hadoop1.${DOMAIN_NAME}' >> /var/spool/cron/root"
